...
class TransactionContentProvider : ContentProvider() {
    init {
        sUriMatcher.addURI(AUTHORITY, TRANSACTION_TABLE, 1);
    }
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    companion object {
        val AUTHORITY = "com.mycelium.spvmodule.contentprovider.TransactionContentProvider"
        val TRANSACTION_TABLE: String = "transaction"
        val TRANSACTION_URL = "content://$AUTHORITY/$TRANSACTION_TABLE"
        val TRANSACTION_CONTENT_URI = Uri.parse(TRANSACTION_URL)


        val _ID = "_id"
        val VALUE = "value"
        val IS_INCOMING = "isIncoming"
        val TIME = "time"
        val HEIGHT = "height"
        val CONFIRMATIONS = "confirmations"
        val IS_QUEUED_OUTGOING = "isQueuedOutgoing"
        val CONFIRMATION_RISK_PROFILE = "confirmationRiskProfile"
        val DESTINATION_ADDRESS = "destinationAddress"
        val TO_ADDRESSES = "toAddresses"

        // Creates a UriMatcher object.
        private val sUriMatcher = UriMatcher(UriMatcher.NO_MATCH)

        private val SELECTION_ACCOUNTINDEX : String = "WHERE accountIndex = ?"
    }
}