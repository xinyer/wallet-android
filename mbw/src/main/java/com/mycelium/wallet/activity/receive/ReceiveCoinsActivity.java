...
public class ReceiveCoinsActivity extends Activity {
    ...
    private void updateUi() {
        ...
        if (CurrencyValue.isNullOrZero(_amount)) {
            ...
            tvAmount.setText(Utils.getFormattedValueWithUnit(getBitcoinAmount(), _mbwManager.getBitcoinDenomination()));
        }
    ...
    public void syncStopped(SyncStopped event) {
        ...
        if (interesting.size() > 0) {
            tvRecv.setText(getString(R.string.incoming_payment) +  Utils.getColuFormattedValueWithUnit(sum));
