...
public class SpvService extends Service {
    private static final String PACKAGE_NAME = SpvService.class.getPackage().getName();
    public static final String ACTION_PEER_STATE = PACKAGE_NAME + ".peer_state";
    public static final String ACTION_PEER_STATE_NUM_PEERS = "num_peers";
    public static final String ACTION_BLOCKCHAIN_STATE = PACKAGE_NAME + ".blockchain_state";
    public static final String ACTION_CANCEL_COINS_RECEIVED = PACKAGE_NAME + ".cancel_coins_received";
    public static final String ACTION_RESET_BLOCKCHAIN = PACKAGE_NAME + ".reset_blockchain";
    public static final String ACTION_BROADCAST_TRANSACTION = PACKAGE_NAME + ".broadcast_transaction";
    public static final String ACTION_BROADCAST_TRANSACTION_HASH = "hash";

    private SpvModuleDashApplication application;
...
//            WalletBalanceWidgetProvider.updateWidgets(SpvService.this, SpvModuleDashApplication.getWallet());
        }
    ...
    private void notifyTransaction(final Transaction tx) {
        ...
        Intent intent = new Intent("com.mycelium.wallet.receivedTransactions");
        intent.putExtra("TRANSACTIONS", txBytes);
        sendBroadcast(intent, "com.mycelium.wallet.RECEIVE_TRANSACTIONS");
    }

    private final class PeerConnectivityListener
            extends AbstractPeerEventListener implements OnSharedPreferenceChangeListener {
        ...
        public void onSharedPreferenceChanged(final SharedPreferences sharedPreferences, final String key) {
            if (Configuration.PREFS_KEY_CONNECTIVITY_NOTIFICATION.equals(key)) {
                changed(peerCount);
            } else if (Configuration.PREFS_KEY_INSTANTX_ENABLED.equals(key)) {
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!connectivityNotificationEnabled || numPeers == 0) {
                        ...
                        final Notification.Builder notification = new Notification.Builder(SpvService.this);
                        notification.setSmallIcon(R.drawable.stat_sys_peers, numPeers > 4 ? 4 : numPeers);
                        notification.setContentTitle(getString(R.string.app_name));
                        notification.setContentText(getString(R.string.notification_peers_connected_msg, numPeers));
                        notification.setContentIntent(PendingIntent.getActivity(SpvService.this, 0, new Intent(SpvService.this,
                                SpvModuleDashApplication.class), 0));
    ...
    private final BroadcastReceiver connectivityReceiver = new BroadcastReceiver() {
        ...
        private void check() {
            final Wallet wallet = SpvModuleDashApplication.getWallet();
            ...
            if (impediments.isEmpty() && peerGroup == null) {
                ...
                final boolean hasTrustedPeer = TextUtils.isEmpty(trustedPeerHost);
    ...
    public void onCreate() {
        ...
        application = (SpvModuleDashApplication) getApplication();
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId) {
        if (intent != null) {
            ...
            if (ACTION_CANCEL_COINS_RECEIVED.equals(action)) {
                ...
                final Sha256Hash hash = Sha256Hash.wrap(intent.getByteArrayExtra(ACTION_BROADCAST_TRANSACTION_HASH));
                final Transaction tx = SpvModuleDashApplication.getWallet().getTransaction(hash);
                log.info("onReceive: TX = " + tx);
                if (peerGroup != null) {
                    log.info("broadcasting transaction " + tx.getHashAsString());
                    peerGroup.broadcastTransaction(tx);
                } else {
                    log.warn("peergroup not available, not broadcasting transaction " + tx.getHashAsString());
                }
            }
        } else {
    ...
    public void onDestroy() {
        ...
        SpvModuleDashApplication.scheduleStartBlockchainService(this);  //disconnect feature

        unregisterReceiver(tickReceiver);

        SpvModuleDashApplication.getWallet().removeEventListener(walletEventListener);

        unregisterReceiver(connectivityReceiver);

        if (peerGroup != null) {
            peerGroup.removeEventListener(peerConnectivityListener);
            peerGroup.removeWallet(SpvModuleDashApplication.getWallet());
            peerGroup.stop();
