...
public class Bip44Account extends AbstractAccount implements ExportableAccount {
  ...
  protected final Bip44AccountBacking _bip44AccountBacking;
  ...
  public synchronized void archiveAccount() {
    ...
    _context.persistIfNecessary(_bip44AccountBacking);
  }
  ...
  protected void initContext(boolean isArchived) {
    ...
    _context.persist(_bip44AccountBacking);
  }
  ...
  protected void ensureAddressIndexes(boolean isChangeChain, boolean full_look_ahead) {
    ...
    for (;index >= 0; index--) {
      ...
      boolean doesAddressExistAlready = _keyManager.doesAddressExistAlready(isChangeChain, index);
      ...
      if(!doesAddressExistAlready) {
        ...
        long timestamp = 1477958400000L; //Test with Nov/01/2016 IN MILLISECONDS // Used to be System.currentTimeMillis();
        ...
        long timestamp = _bip44AccountBacking.getOldestTransactionTimestamp();
        _bip44AccountBacking.storeAddressCreationTime(address, timestamp);
      }
    }
  }
  ...
  private void tightenInternalAddressScanRange() {
    ...
    Collection<TransactionOutputEx> unspent = _bip44AccountBacking.getAllUnspentOutputs();
